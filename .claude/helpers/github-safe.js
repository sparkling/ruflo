 ...
 *   - Strict error handling: all exec* calls inside try/catch; cleanup in
 *     finally; non-zero exit on any error.
 *   - GITHUB_SAFE_DRY_RUN=1 env-var skips the actual `gh` exec for testing.
 *
 * Fork hardening on top of upstream ADR-127 Phase 2:
 *   - execFileSync (not execSync with string interpolation) — args are passed
 *     as an array so shell metacharacters in tmpFile path cannot be exploited.
 *
 * Usage:
