...
hooks:
  pre: |
    ...
    mcp__ruflo__memory_usage store "adaptive:baseline:${TASK_ID}" "$(mcp__ruflo__performance_report --format=json)" --namespace=adaptive
    # Set up real-time monitoring
    mcp__ruflo__swarm_monitor --interval=2000 --swarmId="${SWARM_ID}"
  post: |
    ...
    mcp__ruflo__model_save "adaptive-coordinator-${TASK_ID}" "/tmp/adaptive-model-$(date +%s).json"
    # Update persistent knowledge base
    mcp__ruflo__memory_usage store "adaptive:learned:${TASK_ID}" "$(date): Adaptive patterns learned and saved" --namespace=adaptive
---
