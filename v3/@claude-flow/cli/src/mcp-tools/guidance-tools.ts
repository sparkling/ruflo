  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const refs: Record<string, { title: string; commands: Array<{ cmd: string; desc: string }> }> = {
      'getting-started': {
        ...
        commands: [
          { cmd: 'npx @sparkleideas/cli@latest init --wizard', desc: 'Initialize project with interactive setup' },
          { cmd: 'npx @sparkleideas/cli@latest doctor --fix', desc: 'Run diagnostics and auto-fix issues' },
          { cmd: 'npx @sparkleideas/cli@latest daemon start', desc: 'Start background workers' },
          { cmd: 'npx @sparkleideas/cli@latest status', desc: 'Check system status' },
        ],
      },
      'daily-dev': {
        title: 'Daily Development',
        commands: [
          { cmd: 'npx @sparkleideas/cli@latest hooks pre-task --description "..."', desc: 'Get routing recommendation before task' },
          { cmd: 'npx @sparkleideas/cli@latest hooks post-task --task-id "..." --success true', desc: 'Record task outcome for learning' },
          { cmd: 'npx @sparkleideas/cli@latest hooks post-edit --file "..." --train-neural true', desc: 'Train patterns from edits' },
          { cmd: 'npx @sparkleideas/cli@latest memory search --query "..."', desc: 'Search memory for relevant patterns' },
          { cmd: 'npx @sparkleideas/cli@latest hooks route --task "..."', desc: 'Route task to optimal agent' },
        ],
      },
      'swarm-ops': {
        title: 'Swarm Operations',
        commands: [
          { cmd: 'npx @sparkleideas/cli@latest swarm init --topology hierarchical --max-agents 8', desc: 'Initialize anti-drift swarm' },
          { cmd: 'npx @sparkleideas/cli@latest swarm status', desc: 'Check swarm status' },
          { cmd: 'npx @sparkleideas/cli@latest agent spawn -t coder --name my-coder', desc: 'Spawn a specific agent' },
          { cmd: 'npx @sparkleideas/cli@latest hive-mind init --strategy byzantine', desc: 'Start hive-mind consensus' },
        ],
      },
      'memory-ops': {
        title: 'Memory Operations',
        commands: [
          { cmd: 'npx @sparkleideas/cli@latest memory init --force', desc: 'Initialize memory database' },
          { cmd: 'npx @sparkleideas/cli@latest memory store --key "k" --value "v" --namespace patterns', desc: 'Store a value' },
          { cmd: 'npx @sparkleideas/cli@latest memory search --query "auth patterns"', desc: 'Semantic vector search' },
          { cmd: 'npx @sparkleideas/cli@latest memory list --namespace patterns', desc: 'List entries in namespace' },
          { cmd: 'npx @sparkleideas/cli@latest memory retrieve --key "k" --namespace patterns', desc: 'Get a specific entry' },
        ],
      },
      'github-ops': {
      ...
      diagnostics: {
        ...
        commands: [
          { cmd: 'npx @sparkleideas/cli@latest doctor --fix', desc: 'Full system diagnostics with auto-fix' },
          { cmd: 'npx @sparkleideas/cli@latest status --watch', desc: 'Live system monitoring' },
          { cmd: 'npx @sparkleideas/cli@latest hooks worker status', desc: 'Background worker health' },
          { cmd: 'npx @sparkleideas/cli@latest performance benchmark --suite all', desc: 'Run all benchmarks' },
          { cmd: 'npx @sparkleideas/cli@latest hooks progress --detailed', desc: 'V3 implementation progress' },
        ],
      },
    };
