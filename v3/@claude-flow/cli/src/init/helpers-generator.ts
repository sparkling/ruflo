 ...
 *
 * ADR-0235 §Confirmation #3: emitted as ESM (`.mjs` extension implies module
 * scope under Node — top-level `require()` / `__dirname` are undefined in ESM).
 * Helper modules (router.js, session.js, memory.js, intelligence.cjs) are CJS
 * and loaded via `createRequire(import.meta.url)` to preserve the safeRequire
 * suppress-console-on-load pattern.
 */
...
export function generateIntelligenceStub(): string {
  const lines = [
    ...
    '  // Per-process-unique temp name: a fixed p+".tmp" races when parallel',
    '  // writers share a data dir (the tmp is renamed away by one process,',
    '  // ENOENT on another\'s rename). pid-scoped temp + atomic rename is',
    '  // collision-free; last writer wins on p (POSIX rename is atomic).',
...
/**
 * Generate the check-patches.sh advisory SessionStart hook script.
 *
 * This script is written to .claude/scripts/check-patches.sh (not .claude/helpers/)
 * because settings-generator.ts wires the SessionStart hook to that path:
 *   bash "$(git rev-parse --show-toplevel)"/.claude/scripts/check-patches.sh --global
 *
 * Behavior: advisory fork-health probe. Warns to stderr on drift (detached HEAD,
 * missing 'sparkling' remote, wrong branch). ALWAYS exits 0 so the SessionStart
 * hook never wedges the pipeline. No silent-pass fallbacks — drift surfaces as
 * visible [check-patches] stderr warnings.
 *
 * NOTE: This content MUST remain byte-identical to the copy shipped in
 * ruflo-patch's .claude/scripts/check-patches.sh (paired-fix rule). If you
 * change one, change the other in the same commit.
 */
export function generateCheckPatchesScript(): string {
  // Use a raw string: no interpolation so bash $-expansions survive untouched.
  return [
    '#!/usr/bin/env bash',
