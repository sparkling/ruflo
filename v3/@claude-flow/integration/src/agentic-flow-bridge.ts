  ...
  private async doInitialize(config?: Partial<IntegrationConfig>): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.warn('[AgenticFlowBridge] SDK bridge initialization failed, continuing without it:', (sdkError as Error).message);
      ...
      if (this.config.features.enableSONA) {
        try {
          ...
          console.warn('[AgenticFlowBridge] SONA initialization failed, continuing without it:', (sonaError as Error).message);
          this.sona = null;
      ...
      if (this.config.features.enableFlashAttention) {
        try {
          ...
          console.warn('[AgenticFlowBridge] Attention initialization failed, continuing without it:', (attError as Error).message);
          this.attention = null;
          this.updateComponentHealth('attention', 'degraded', (attError as Error).message);
        }
      }
