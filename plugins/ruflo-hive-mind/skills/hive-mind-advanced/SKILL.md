...
# Force reinitialize
npx @sparkleideas/cli@latest hive-mind init --force

# Custom configuration
npx @sparkleideas/cli@latest hive-mind init --config hive-config.json
```
...
npx @sparkleideas/cli@latest hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx @sparkleideas/cli@latest hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx @sparkleideas/cli@latest hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
npx @sparkleideas/cli@latest hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Claude Code commands
npx @sparkleideas/cli@latest hive-mind spawn "Build full-stack app" --claude
```
...
npx @sparkleideas/cli@latest hive-mind metrics

# Monitor collective memory
npx @sparkleideas/cli@latest hive-mind memory
```
...
npx @sparkleideas/cli@latest hive-mind sessions

# Pause a session
npx @sparkleideas/cli@latest hive-mind pause <session-id>

# Resume a paused session
npx @sparkleideas/cli@latest hive-mind resume <session-id>

# Stop a running session
npx @sparkleideas/cli@latest hive-mind stop <session-id>
```
...
```bash
npx @sparkleideas/cli@latest hive-mind spawn "Build REST API" --claude
```
...
npx @sparkleideas/cli@latest sparc tdd "User authentication" --hive-mind
...
npx @sparkleideas/cli@latest hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
npx @sparkleideas/cli@latest hive-mind spawn "Review PR #123" --queen-type tactical
```
...
```bash
npx @sparkleideas/cli@latest hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
npx @sparkleideas/cli@latest hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
npx @sparkleideas/cli@latest hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx @sparkleideas/cli@latest hive-mind memory --gc

# Optimize database
npx @sparkleideas/cli@latest hive-mind memory --optimize

# Export and clear
npx @sparkleideas/cli@latest hive-mind memory --export --clear
```
...
npx @sparkleideas/cli@latest hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx @sparkleideas/cli@latest hive-mind spawn "..." --consensus majority
```
...
```bash
# Frontend hive
npx @sparkleideas/cli@latest hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx @sparkleideas/cli@latest hive-mind spawn "Build API" --name backend-hive
...
npx @sparkleideas/cli@latest hive-mind export <session-id> --output backup.json

# Import session
npx @sparkleideas/cli@latest hive-mind import backup.json
```
...
npx @sparkleideas/cli@latest hive-mind spawn "Build e-commerce platform" \
...
```bash
# Spawn research hive
npx @sparkleideas/cli@latest hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx @sparkleideas/cli@latest hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
