...
1. **train** — `mcp__ruflo__neural_train` with `--pattern-type` (`coordination|edit|task`) and `--epochs N`.
...
6. **compress** — `mcp__ruflo__neural_compress` to compact stored patterns (storage efficiency, runs after consolidation).
