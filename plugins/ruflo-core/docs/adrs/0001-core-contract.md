...
- `.mcp.json` — registers `ruflo` MCP server via `npx -y @sparkleideas/cli@latest`
...
2. **No smoke test.**
3. **No Compatibility section** pinning to `@sparkleideas/cli` v3.6.
...
- **Compatibility** — pin to `@sparkleideas/cli` v3.6. Note the `npx -y @sparkleideas/cli@latest` invocation in `.mcp.json` is the dynamic resolver; smoke verifies the resolved version.
...
- `v3/@sparkleideas/cli/` — the MCP server source backing this plugin
