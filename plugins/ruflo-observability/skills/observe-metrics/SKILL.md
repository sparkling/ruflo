...
1. **Retrieve metrics** -- call `mcp__ruflo__agentdb_hierarchical-recall` to fetch metric records from the `observability` namespace for the specified period (default: 1 hour)
2. **Aggregate** -- compute:
...
3. **Compute baselines** -- search for historical patterns via `mcp__ruflo__agentdb_pattern-search` to establish baseline values for each metric
4. **Flag anomalies** -- mark metrics deviating >2 standard deviations from baseline with direction (above/below) and severity
5. **Store patterns** -- call `mcp__ruflo__agentdb_pattern-store` to record current metric snapshot for future baseline comparison
...
```bash
npx @sparkleideas/cli@latest memory search --query "system metrics for last hour" --namespace observability
```
