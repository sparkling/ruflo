...
```bash
npx @sparkleideas/cli@latest memory store --namespace plugin-patterns --key "plugin-TYPE" --value "STRUCTURE_AND_CONFIG"
npx @sparkleideas/cli@latest memory search --query "plugin scaffold for TYPE" --namespace plugin-patterns
```
