...
allowed-tools: mcp__ruflo__agentdb_hierarchical-recall mcp__ruflo__agentdb_pattern-store mcp__ruflo__agentdb_pattern-search mcp__ruflo__ruvllm_hnsw_route Bash
---
...
1. **Load candles** -- call `mcp__ruflo__agentdb_hierarchical-recall` to retrieve normalized OHLCV data for the symbol and period from `market-data` namespace
...
5. **Store** -- call `mcp__ruflo__agentdb_pattern-store` to persist each detected pattern in `market-patterns` namespace with the pattern vector
