...
argument-hint: ""
allowed-tools: Read Glob Grep Bash mcp__ruflo__agentdb_hierarchical-recall mcp__ruflo__agentdb_pattern-store mcp__ruflo__agentdb_semantic-route
---
...
1. **Find pending migrations** -- use `Glob` to list all migration files, cross-reference with applied history via `mcp__ruflo__agentdb_hierarchical-recall` to identify pending ones
...
9. **Store results** -- call `mcp__ruflo__agentdb_pattern-store` to record validation patterns for future reference
