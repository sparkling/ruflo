...
- `mcp__ruflo__agentdb_hierarchical-store` -- store migration metadata and history
- `mcp__ruflo__agentdb_hierarchical-recall` -- recall migration status and history
- `mcp__ruflo__agentdb_pattern-store` -- store successful migration patterns
- `mcp__ruflo__agentdb_pattern-search` -- search for similar migration patterns
- `mcp__ruflo__agentdb_semantic-route` -- route queries to relevant schema documentation
...
npx @sparkleideas/cli@latest neural train --pattern-type migrations --epochs 10
```
...
```bash
npx @sparkleideas/cli@latest memory store --namespace migrations --key "migration-NNN_NAME" --value "MIGRATION_METADATA_JSON"
npx @sparkleideas/cli@latest memory store --namespace migration-patterns --key "pattern-PATTERN_NAME" --value "PATTERN_JSON"
npx @sparkleideas/cli@latest memory search --query "migrations adding foreign keys" --namespace migrations
```
