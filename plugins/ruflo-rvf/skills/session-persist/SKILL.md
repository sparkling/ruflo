...
allowed-tools: mcp__ruflo__session_save mcp__ruflo__session_restore mcp__ruflo__session_list mcp__ruflo__session_info mcp__ruflo__session_delete mcp__ruflo__hooks_session-start mcp__ruflo__hooks_session-end mcp__ruflo__hooks_session-restore Bash
---
...
1. **Save session** — call `mcp__ruflo__session_save` to snapshot current state
2. **List sessions** — call `mcp__ruflo__session_list` to see all saved sessions
3. **Restore** — call `mcp__ruflo__session_restore` to resume a previous session
4. **Info** — call `mcp__ruflo__session_info` for session details and metadata
5. **Clean up** — call `mcp__ruflo__session_delete` to remove old sessions
