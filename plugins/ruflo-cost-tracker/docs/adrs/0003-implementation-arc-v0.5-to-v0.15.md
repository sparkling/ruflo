...
4. **P4 — `compact.mjs` (v0.8.0)** — dropped the inline `node --input-type=module -e '...'` block from `cost-compact-context`. True MCP wrapping (modifying @sparkleideas/cli source) deliberately deferred — see "Riskiest assumption" below.
...
- **No real MCP tools registered** — adding `cost_report` / `cost_summary` MCP tools requires modifying `v3/@sparkleideas/cli/src/mcp-tools/`, which is outside plugin-local scope and deserves its own ADR. The current `summary.mjs` provides equivalent functionality via Bash shell-out, but it is *not* an MCP tool.
- **Budget upsert workaround** — `npx @sparkleideas/cli memory store` rejects keys that `memory retrieve` doesn't see (a UNIQUE-constraint inconsistency in the @sparkleideas/cli memory layer). `budget.mjs` works around this by writing timestamped keys (`budget-config-<ms>`) and resolving the latest at retrieve time. This is functional but indicates an upstream bug that should be fixed in a separate ADR.
