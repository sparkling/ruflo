---
name: embeddings
description: RuVector embedding engine status and operations -- ONNX, HNSW
---
...
5. For hierarchical data (taxonomies, code trees), use `mcp__ruflo__embeddings_hyperbolic` (Poincare ball model).
6. The substrate-level entry point `mcp__ruflo__embeddings_neural` exists; in normal use it's covered by `embeddings_init` + `embeddings_generate`.
