...
3. **Wait** — call `mcp__ruflo__browser_wait` for elements to appear or network idle
4. **Validate** — call `browser_get-text` / `browser_get-value` to check content
5. **Screenshot** — call `mcp__ruflo__browser_screenshot` to capture visual state
6. **Snapshot** — call `mcp__ruflo__browser_snapshot` for accessibility tree
7. **Clean up** — call `mcp__ruflo__browser_close` when done
