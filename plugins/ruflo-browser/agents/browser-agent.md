...
Use these MCP tools:
- `mcp__ruflo__browser_open` / `browser_close` for session management
- `mcp__ruflo__browser_click` / `browser_fill` / `browser_type` for interaction
- `mcp__ruflo__browser_screenshot` / `browser_snapshot` for capture
- `mcp__ruflo__browser_get-text` / `browser_eval` for extraction
- `mcp__ruflo__browser_wait` for synchronization
...
```bash
npx @sparkleideas/cli@latest memory store --namespace browser-patterns --key "site-DOMAIN" --value "SELECTORS_AND_STRATEGY"
npx @sparkleideas/cli@latest memory search --query "scraping pattern for DOMAIN" --namespace browser-patterns
```
