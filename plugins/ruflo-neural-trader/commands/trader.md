...
2. Store strategy config in memory:
   `npx @sparkleideas/cli@latest memory store --key "strategy-NAME" --value "CONFIG" --namespace trading-strategies`
...
3. Store results:
   `npx @sparkleideas/cli@latest memory store --key "backtest-ID" --value "RESULTS" --namespace trading-backtests`
...
3. Store signals:
   `npx @sparkleideas/cli@latest memory store --key "signal-TIMESTAMP" --value "SIGNALS" --namespace trading-signals`
...
3. Store assessment:
   `npx @sparkleideas/cli@latest memory store --key "risk-ID" --value "METRICS" --namespace trading-risk`
...
3. Store allocation:
   `npx @sparkleideas/cli@latest memory store --key "portfolio-TIMESTAMP" --value "ALLOCATION" --namespace trading-portfolio`
...
**history**:
1. Search memory: `npx @sparkleideas/cli@latest memory search --query "trade history" --namespace trading-history`
