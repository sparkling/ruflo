...
allowed-tools: Bash Read mcp__ruflo__memory_store mcp__ruflo__memory_retrieve mcp__ruflo__memory_search mcp__ruflo__neural_predict mcp__ruflo__agentdb_pattern-search
...
2. Load current portfolio:
   `mcp__ruflo__memory_search({ query: "current portfolio holdings", namespace: "trading-portfolio" })`
...
5. Use SONA for expected return prediction:
   `mcp__ruflo__neural_predict({ input: "expected returns for [HOLDINGS] given current regime" })`
...
7. Search for similar allocations in history:
   `mcp__ruflo__agentdb_pattern-search({ query: "optimized portfolio Sharpe > 1", namespace: "trading-portfolio" })`
8. Store optimized allocation:
   `mcp__ruflo__memory_store({ key: "portfolio-optimal-TIMESTAMP", value: "ALLOCATION_JSON", namespace: "trading-portfolio" })`
