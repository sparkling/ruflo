...
Use these MCP tools:
- `mcp__ruflo__wasm_agent_create` / `wasm_agent_terminate` for lifecycle
- `mcp__ruflo__wasm_agent_prompt` / `wasm_agent_tool` for interaction
- `mcp__ruflo__wasm_agent_files` / `wasm_agent_export` for data management
- `mcp__ruflo__wasm_gallery_*` for gallery operations
...
```bash
npx @sparkleideas/cli@latest memory store --namespace wasm-patterns --key "agent-TYPE" --value "CONFIG_AND_PERFORMANCE"
npx @sparkleideas/cli@latest memory search --query "wasm agent for TASK" --namespace wasm-patterns
```
