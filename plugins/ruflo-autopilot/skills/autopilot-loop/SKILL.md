...
argument-hint: ""
allowed-tools: mcp__ruflo__autopilot_status mcp__ruflo__autopilot_predict mcp__ruflo__autopilot_log mcp__ruflo__autopilot_progress mcp__ruflo__autopilot_disable ScheduleWakeup Agent
---
Run one autopilot iteration using Claude Code's native /loop:

1. Check status: `mcp__ruflo__autopilot_status`
2. If all tasks complete or max iterations reached, call `mcp__ruflo__autopilot_disable` and stop
3. Get prediction: `mcp__ruflo__autopilot_predict` for the optimal next action
4. Execute the predicted task (spawn agent, edit code, run tests, etc.)
5. Log via `mcp__ruflo__autopilot_log`
...
Configure: `mcp__ruflo__autopilot_config({ taskSources: ["team-tasks", "swarm-tasks"] })`
