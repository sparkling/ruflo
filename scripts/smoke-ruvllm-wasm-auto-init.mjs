...
for (const name of wasmTouchingTools) {
  ...
  // create handler already did the init. Fork additionally accepts the
  // `getOrRebuild*` wrappers (rebuildSona/rebuildHnswRouter/rebuildMicroLora
  // each call loadRuvllmWasm internally before re-creating the instance).
  ...
  const usesRebuildWrapper = /(?:getOrRebuild(?:Sona|Hnsw|MicroLora))\b/.test(block[0]);
  if (!usesAutoInit && !usesInstanceLookup && !usesRebuildWrapper) {
    fail(`${name} bypasses loadRuvllmWasm() AND has no instance lookup — #2086 regression`);
  } else {
    pass(`${name} ${usesAutoInit ? 'auto-inits via loadRuvllmWasm()' : (usesInstanceLookup ? 'uses prior instance from create handler' : 'uses getOrRebuild wrapper (auto-inits on rebuild path)')}`);
  }
}
